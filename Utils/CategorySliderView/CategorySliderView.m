...
- (void)addCategotyView:(UIView *)view {
    ...
    if (self.sliderDirection == SliderDirectionHorizontal) {
        ...
        else if(self.categoryViews.count == 0){
            w = self.categoryViewPadding/2;
        }
        else {
    ...
    else if(SliderDirectionLineFeed){
        float w = 0;
        float h = 0;
        
        if(self.categoryViews.count % 4 == 0 && self.categoryViews.count){
            if (self.categoryViews.count > 0) {
                UIView *lastView = [self.categoryViews lastObject];
                h = lastView.frame.origin.x+lastView.frame.size.width+self.categoryViewPadding;
            }
            else if(self.categoryViews.count == 0){
                h = self.categoryViewPadding/2;
            }
            else {
                h = [self width]/2 - view.frame.size.width/2;
            }
        }else{
            if (self.categoryViews.count > 0) {
                UIView *lastView = [self.categoryViews lastObject];
        ...
        [self.scrollView addSubview:view];
        [view setFrame:CGRectMake(w, h, view.frame.size.width, view.frame.size.height)];
